...
import { ISearchResult } from '../interfaces/IStreaming';
...
/**
 * Modal component for displaying search results
 */
const SearchResultsModal = ({ isOpen, results, onClose, onSelectResult }: SearchResultsModalProps) => {
  return (
                  ...
                  {results.length > 0 ? (
                    <div className="space-y-4">
                      {results.slice(0, 20).map((result, index) => (
                        ...
                        >
                          <p className="font-medium text-slate-900 dark:text-slate-100">{result.text}</p>
                          <p className="text-sm text-slate-600 dark:text-slate-400">{result.base_domain}</p>
                        </button>
                      ))}
                    </div>
                  ) : (
