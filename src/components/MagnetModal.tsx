import { Fragment } from 'react';
import { Dialog, Transition } from '@headlessui/react';
import { IMagnetLink } from '../interfaces/IStreaming';

interface MagnetModalProps {
  isOpen: boolean;
  magnets: IMagnetLink[];
  onClose: () => void;
  onSelectMagnet: (magnet: IMagnetLink) => void;
}

/**
 * Modal component for displaying magnet links
 */
const MagnetModal = ({ isOpen, magnets, onClose, onSelectMagnet }: MagnetModalProps) => {
  return (
    <Transition appear show={isOpen} as={Fragment}>
      <Dialog as="div" className="relative z-[999999]" onClose={onClose}>
        <Transition.Child
          as={Fragment}
          enter="ease-out duration-300"
          enterFrom="opacity-0"
          enterTo="opacity-100"
          leave="ease-in duration-200"
          leaveFrom="opacity-100"
          leaveTo="opacity-0"
        >
          <div className="fixed inset-0 bg-black/50 backdrop-blur" />
        </Transition.Child>

        <div className="fixed inset-0 overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            <Transition.Child
              as={Fragment}
              enter="ease-out duration-300"
              enterFrom="opacity-0 scale-95"
              enterTo="opacity-100 scale-100"
              leave="ease-in duration-200"
              leaveFrom="opacity-100 scale-100"
              leaveTo="opacity-0 scale-95"
            >
              <Dialog.Panel className="w-full max-w-md p-4 text-left align-middle rounded-xl bg-background-light dark:bg-slate-900 shadow-xl transform overflow-hidden transition-all">
                <Dialog.Title className="text-lg font-medium text-slate-900 dark:text-slate-100">
                  Select Magnet Link
                </Dialog.Title>
                <div className="mt-4 max-h-[60vh] overflow-y-auto scroll-thin">
                  {magnets.length > 0 ? (
                    <div className="space-y-4">
                      {magnets.map((magnet, index) => (
                        <button
                          key={index}
                          className="w-full p-3 text-left rounded-md hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors"
                          onClick={() => onSelectMagnet(magnet)}
                        >
                          <p className="font-medium text-slate-900 dark:text-slate-100">{magnet.display_name}</p>
                          <p className="text-sm text-slate-600 dark:text-slate-400">Size: {magnet.file_size_formatted}</p>
                        </button>
                      ))}
                    </div>
                  ) : (
                    <div className="h-20 flex justify-center items-center text-slate-600 dark:text-slate-400">
                      No magnet links found
                    </div>
                  )}
                </div>
              </Dialog.Panel>
            </Transition.Child>
          </div>
        </div>
      </Dialog>
    </Transition>
  );
};

export default MagnetModal;