            ...
            async with AsyncWebCrawler() as crawler:
                ...
                if result and hasattr(result, "markdown") and result.markdown:
                    ...
                    magnet_matches = re.findall(
                        magnet_pattern, result.markdown, re.IGNORECASE
                    )

                    if magnet_matches:
                        print(
                            f"✅ Found {len(magnet_matches)} magnet links with {location['name']}"
                        )
                        ...
                        for magnet_url in magnet_matches:
                            ...
                            if parsed_magnet["file_size"]:
                                parsed_magnet["file_size_formatted"] = format_file_size(
                                    parsed_magnet["file_size"]
                                )
                            else:
